...
class Zoom
{
    ...
    // Added Function (not built-in function) HTN
    ...
    public function deleteAllMeetings()
    {
        try {
            ...
            foreach ($data['meetings'] as $meeting) {
                ...
                $deleteResponse = $this->client->request('DELETE', 'meetings/' . $meetingId);

                if ($deleteResponse->getStatusCode() !== 204) {
                    // Handle failed deletion (e.g., log error, retry, or notify user)
                    return [
                        'status' => false,
                        'message' => 'Error deleting meeting: ' . $meetingId,
                    ];
                }
            }
